...
class RNN_Model():
  ...
  def add_model_vars(self, keep_prob):
    '''
    ...
    with tf.variable_scope('Composition'):
      ...
      lstm_cell_forward = tf.nn.rnn_cell.LSTMCell(self.config.embed_size)
      lstm_cell_backward = tf.nn.rnn_cell.LSTMCell(self.config.embed_size)
    ...
    with tf.variable_scope('Projection'):
      tf.get_variable('U', [2*self.config.embed_size, self.config.label_size])
  ...
  def add_model(self, node):
    ...
    # curr_node_dep_emb = self.embed_word(node.dep)
      ...
      if not node.isLeaf:
        left_state = node_tensors[node.left][1]
      ...
      out_wordfw, state_wordfw = fw_cell(right_state, left_state)

      # out_wordbw, state_wordbw = bw_cell(left_state, right_state)
      ...
      out = tf.concat(1, [out_wordfw, out_wordbw])
      state = tf.concat(1, [state_wordfw, state_wordbw])
      osTuple = (out, state)
